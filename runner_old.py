...
class Runner:
    ...
    def fq(self, db, table):
        return f"{db}.{self.env['schema']}.{table}"
    ...
    def columns(self, db, table):
        ...
        rows = self.q(f"""
            ...
            WHERE table_schema = '{self.env['schema']}' AND table_name = '{table}'
    ...
    def t0(self, table):
        prod_db, uat_db = self.env["prod_db"], self.env["uat_db"]
        rows = self.q(f"""
            ...
            FROM (SELECT * FROM {prod_db}.INFORMATION_SCHEMA.COLUMNS
                  WHERE table_schema = '{self.env['schema']}' AND table_name = '{table}') p
            FULL JOIN (SELECT * FROM {uat_db}.INFORMATION_SCHEMA.COLUMNS
                  WHERE table_schema = '{self.env['schema']}' AND table_name = '{table}') u
